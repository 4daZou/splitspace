...
@Component({
  ...
  styles: [`
    ...
    .checkbox-group { display: flex; gap: 0.75rem; flex-wrap: wrap; margin-top: 0.25rem; }
    .checkbox-item { display: flex; align-items: center; gap: 0.4rem; cursor: pointer; font-size: 0.9rem; }
    .checkbox-item input { width: 16px; height: 16px; accent-color: #6c8cff; flex-shrink: 0; }
    .split-preview { margin-top: 0.5rem; font-size: 0.9rem; color: #555; background: #6c8cff0d; padding: 0.5rem 0.75rem; border-radius: 6px; }
    .form-actions { display: flex; gap: 0.75rem; justify-content: flex-end; margin-top: 0.5rem; flex-wrap: wrap; }
...
export class AddExpenseComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private expenseService: ExpenseService, private roommateService: RoommateService, private router: Router) {}
  ...
  onSplitChange(event: Event, id: string) {
    ...
    this.selectedRoommates = checked
      ? [...this.selectedRoommates, id]
      : this.selectedRoommates.filter(rid => rid !== id);
  }

  onSubmit() {
    if (this.expenseForm.invalid) return;
    this.submitting = true;
    this.expenseService.create({ ...this.expenseForm.value, splitBetween: this.selectedRoommates }).subscribe({
